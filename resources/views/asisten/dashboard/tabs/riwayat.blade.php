    <div class="welcome-banner" style="margin-bottom: 2rem;">
        <div class="status-badge" style="background: rgba(234, 179, 8, 0.1); color: #eab308; border-color: rgba(234, 179, 8, 0.3);">
            ...
            {{ $lang === 'id' ? 'Jejak Audit & Akuntabilitas' : 'Audit Trail & Accountability' }}
        </div>
        <h1 class="welcome-title">{{ $lang === 'id' ? 'Riwayat Aktivitas' : 'Audit Logs' }}</h1>
        <p class="welcome-subtitle">
            {{ $lang === 'id' ? 'Seluruh riwayat persetujuan, penolakan, maupun pengembalian aset dari antrean sistem yang terekam secara permanen.' : 'All recorded history of approvals, rejections, and asset returns within the system queue permanently.' }}
        </p>
    </div>
    ...
    <div style="background: rgba(10, 16, 22, 0.5); border: 1px solid var(--panel-border); border-radius: 1rem; padding: 1.5rem;">
        ...
        @else
            <div style="text-align: center; padding: 4rem 1rem;">
                <p style="color: var(--text-muted); font-size: 0.95rem;">{{ $lang === 'id' ? 'Belum ada rekaman jejak riwayat di laboratorium Anda.' : 'No audit trail recorded in your laboratory yet.' }}</p>
            </div>
        @endif
    </div>
