        ...
        <div class="modal-content" style="border-color: #0ea5e9;">
            ...
            <form id="decisionForm" method="POST">
                ...
                <div class="modal-form-group">
                    <label>{{ $lang === 'id' ? 'CATATAN MASTER LAB (OPSIONAL)' : 'LAB MASTER NOTES (OPTIONAL)' }}</label>
    ...
    <script>
        function approveModal(action, id) {
            ...
            form.action = '/master/dashboard/approvals/' + id;
            
            if(action === 'approve') {
                titleEl.innerText = "{{ $lang === 'id' ? 'OTORISASI PERSETUJUAN' : 'AUTHORIZE APPROVAL' }}";
                titleEl.style.color = '#22c55e';
                submitBtn.style.background = '#22c55e';
                submitBtn.innerText = "{{ $lang === 'id' ? 'BERIKAN ACC' : 'GRANT APPROVAL' }}";
            } else {
                titleEl.innerText = "{{ $lang === 'id' ? 'EKSEKUSI PENOLAKAN' : 'EXECUTE REJECTION' }}";
                titleEl.style.color = '#ef4444';
                submitBtn.style.background = '#ef4444';
                submitBtn.innerText = "{{ $lang === 'id' ? 'TOLAK PERMOHONAN' : 'REJECT REQUEST' }}";
            }
